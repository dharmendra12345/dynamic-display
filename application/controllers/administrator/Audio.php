...
function __construct(){
    ...
    public function index() {
		$data['title'] = 'Section';
		$data['active_tab_sound'] = 'active';
	    $data['error'] = '';		
		$data['audio'] = $this->Common_model->getAll('audio_item_master');
	    $this->load->view('administrator/include/inner_header',$data);
	    $this->load->view('administrator/audio/index',$data);
	    $this->load->view('administrator/include/inner_footer');
    }
	public function add_audio(){  
		 $data['title'] = 'Add audio';
		 $data['heading'] = 'Add';
		 $this->form_validation->set_rules('sound_path','User Id','trim|required');	
		 if($this->form_validation->run() == 'true'){ 
			...
			$id = $this->Common_model->insertData('audio_item_master',$array);
			...
			//$this->Common_model->insertData('audio_item1',$array1);
			$this->session->set_flashdata('item','Sound Create Successfully');
		    redirect('administrator/manage-audio');
			}
		$this->load->view('administrator/include/inner_header',$data);
	    $this->load->view('administrator/audio/addaudio',$data);
	...
	public function edit_cell(){
	 ...
	 public function view_audio(){
	     ...
	     $ids = $this->uri->segment('3');
		   $data['singledata'] = $this->Common_model->getsingle('audio_item_master',array('sound_id' => $this->uri->segment('3')));
	     $this->load->view('administrator/include/inner_header',$data);
	...
	public function delete_audio(){
	   ...
	   $ids = $this->uri->segment('3');	  
	   $deleteData = $this->Common_model->delete('audio_item_master',array('sound_id' => $ids));
