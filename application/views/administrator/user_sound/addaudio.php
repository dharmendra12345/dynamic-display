	  ...
	  <?php }?>
                 	...
                 	<div class="panel panel-default">
                        ...
                        <div class="panel-body">
							 <form  class="form-horizontal"  method="post" <?php if(empty($singledata)){ ?> action="<?php echo base_url(); ?>administrator/addUserAudio" <?php }else{ ?> action="<?php echo base_url(); ?>administrator/edit-cell/<?php echo $singledata->sound_id; ?>" <?php } ?> id="schedule_data" enctype="multipart/form-data">
						 ...
						 <div class="form-group">
                                <label class="col-sm-2 control-label">Select Sound  </label>
                                <div class="col-sm-7">
                                   <select class="form-control" name="sound"> 
									 ...
									 <?php 
									   foreach($audio_item as $val) { ?> 
                             	      <option value="<?php echo $val->sound_id;?>" <?php if(!empty($singledata) and $val->sound_id==$singledata->sound_id){ echo "selected";} ?> > 
									   <?php echo $val->sound_path;?>
									 </option>
									  <?php } ?>
								   </select>
                                </div>
