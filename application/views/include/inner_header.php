            ...
            <nav class="navbar navbar-default" role="navigation">
                <div class="navigation">
                    ...
                    <div class="collapse navbar-collapse hdr_clps_blck" id="bs-example-navbar-collapse-1">
						...
						<form method="post" action="<?php echo base_url().'search-tournament/';?>" name="search">
                            <ul class="nav navbar-nav">
                                ...
                                <li class="search-li">
                                    <div class="input-append">
										...
										<button class="add-on pull-right" name="search" type="submit"><img src="<?php echo base_url();?>assets/images/srch_head2.png" /></button>
                                    </div>
                                </li>
                            </ul>
                          </form>  
						<?php  }else{  ?>
						 <form method="post" action="<?php echo base_url().'search-users/';?>" name="search">
                            <ul class="nav navbar-nav">
                                <li class="active li_first"><a href="<?php echo base_url(); ?>tournaments">Tournament</a></li>
								<?php $tournament = $this->Common_model->getAll('tournament'); ?>
                                <li class="li_sec"><a href="#" class="all-br"><?php echo count($tournament); ?></a></li>
                                <li class="search-li">
                                    <div class="input-append">
										...
										<!--<a href="#"> <span class="add-on"><i class="fa fa-search"></i></span></a>-->
										<button class="add-on pull-right" name="search_sub" type="submit"><i class="fa fa-search"></i></button>
                                    </div>
                                </li>
                            </ul>
                          </form>  
								...
								<?php $notification_data_val = $this->Common_model->getAllwhere('notifications',array('is_read' => 0,'type' => 'ACCEPTREQUEST','friend_user_id' => $this->session->userdata('userid'))); //echo '<pre>';print_r($notification_data_val); ?>			
									...
									<div class="req3">
                                        <div class="requests req4">
											...
											$friend_notification = $this->Common_model->jointwotable('users','id','friends','friend_user_id',array('friends.user_id' => $this->session->userdata('userid'),'friends.request_status' => 1),'users.image,friends.friend_id,users.username,friends.friend_user_id,friends.user_id');
											//echo '<pre>';print_r($friend_notification);
											if(empty($friend_request) && (!$this->input->is_ajax_request())){
												$this->session->set_flashdata('notfound','Friend Request not available');
											}
											
											if(!empty($friend_notification)){
												...
												<?php } ?>
												 ...
												 <div class="requ-data">
                                                    <p><span class="dark-name"><?php echo ucwords($notification->username); ?></span> and <span class="dark-name">Me</span> are now friends</p>
											   </div>  
											   </div>
